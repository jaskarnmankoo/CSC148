...
class SudokuPuzzle(Puzzle):
    ...
    def fail_fast(self):
        """
        Returns whether or not it is possible to continue the current
        sudoku puzzle
        @type self: SudokuPuzzle
        @rtype: bool
        ...
        True
        """
        # Get the index of empty place
        i = -1
        for value in self._symbols:
            i += 1
            if value == '*':

                # The difference between the _symbol_set and the union of
                # the row, column, and sub_square of the empty position will
                # output a set of legal symbols to place in the empty spot
                allowed_symbols = self._symbol_set - \
                    (self._row_set(i) |
                     self._column_set(i) |
                     self._subsquare_set(i))
                # If there are no legal symbols, it's impossible to continue
                if len(allowed_symbols) == 0:
                    return True
                    # Runs loop again to check for more "*" that satisfy this
        return False
